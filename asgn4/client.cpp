...
int32_t main(int32_t argc, char **argv) {
  ...
  for (int i = 2; i < argc; i++) {
     ...
     * a:existing_name:new_name -> PATCH existing_name HTTP/1.1\r\n\r\n
    ...
    if (strcmp(curr_argv[0], "s") == 0) {
      ...
      // receive continue info from client
      memset(headerBuf, 0, 4096);
      recv(sock, headerBuf, sizeof(headerBuf), 0);
      ...
      memset(headerBuf, 0, 4096);
      count = recv(sock, headerBuf, sizeof(headerBuf), 0);
      // fprintf(stdout, "%s\r\n\r\n", headerBuf);
      close(sock);
      close(fd);
      continue;
    ...
    if (strcmp(curr_argv[0], "r") == 0) {
      ...
      if ((fd = open(curr_argv[2], O_WRONLY | O_TRUNC)) == -1) {
        if ((fd = open(curr_argv[2], O_CREAT | O_WRONLY | O_TRUNC, S_IRWXU)) ==
            -1) {
          fprintf(stderr, "Error: fail open: %s\n", strerror(errno));
    ...
    if (strcmp(curr_argv[0], "a") == 0) {
      // send PATCH request header to server
      // existing_name is curr_argv[1]
      // new_name is curr_argv[2]
      strcat(headerBuf, "PATCH ");
      strcat(headerBuf, curr_argv[1]);
      strcat(headerBuf, " HTTP/1.1\r\n\r\n");
      // send body: request ALIAS
      strcat(headerBuf, "ALIAS ");
      strcat(headerBuf, curr_argv[1]);
      strcat(headerBuf, " ");
      strcat(headerBuf, curr_argv[2]);
      strcat(headerBuf, "\r\n\0");
      ...
      // receive PATCH response header from server
      memset(headerBuf, 0, 4096);
      if ((count = recv(sock, headerBuf, sizeof(headerBuf), 0)) == 0) {
        fprintf(stderr, "Error: no response header\n");
      }
